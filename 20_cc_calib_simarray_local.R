...
#! lst<-list(c(mean=-150,sd=100),c(mean=-50,sd=100))
#! sapply(lst, function(x) rnorm(1,lst[[1]],lst[[2]]) )
#! mapply(function(x,y) rnorm(1,x,y), c(100,-50),c(1,50))
#! a3[1,c(4,6)]
#! a3[1,c(5,7)]
...
theta_2s <- c(0.1) # poly-serial correlation in treatment group

#! mu_e2s <-mu_e1 + c(0, 100, 150) # effectiveness in trt
mu_e2s <-mu_e1 + c(0, 100) # effectiveness in trt
sigma2_e2s <- 100^2
...
saveRDS(cb_calib_simarray, file = file.path(getwd(),"cb_calib_simarray_local.rds"))
